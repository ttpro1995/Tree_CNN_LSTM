...
Adapt from https://github.com/stanfordnlp/treelstm by meow
With sequential  mode
seq_sents are all non-dublicate entries where
  - train: text span
  - dev: full sentences only
  - test: full sentences only
"""
...
def write_labels(dirpath, dictionary, get_span = False):
    '''

    :param dirpath:
    :param dictionary:
    :param get_span: False - seq only full sentences, for dev/test. True then get span (removed dub)
    :return:
    '''
    print('Writing labels for trees in ' + dirpath)
    with open(os.path.join(dirpath, 'labels.txt'), 'w') as labels, \
        open(os.path.join(dirpath, 'seq_labels.txt'), 'w') as seq_labels, \
        open(os.path.join(dirpath, 'seq_sents.txt'), 'w') as seq_sents:
        ...
        for i in xrange(len(const_trees)):
            ...
            if get_span:
                span_list.extend(s)
            else: # not get span, full sentences only
                span_list.append(const_trees[i].span)

            labels.write(' '.join(map(str, l)) + '\n') # write tree label

        # span_set = set(span_list) # remove dublicate
        span_set = span_list # no remove dublicate
        # print dublicate
        # import collections
        # print [item for item, count in collections.Counter(span_list).items() if count > 1]

        for span in span_set:
            seq_sents.write(span+'\n') # write span
            seq_labels.write(str(dictionary[span])+'\n') # write label of the span
...
if __name__ == '__main__':
    ...
    write_labels(train_dir, dictionary, get_span=True) # will get text span for seq training
